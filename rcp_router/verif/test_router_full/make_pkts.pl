...
$delay = '@120us';
#nf_PCI_read32($delay, $batch, CPU_REG_Q_1_RD_NUM_WORDS_AVAIL_REG(), 3*16);
#$delay = 0;
#nf_PCI_read32($delay, $batch, CPU_REG_Q_1_RD_NUM_PKTS_IN_Q_REG(), 3);
cpu_rxfifo_rd_pkt($in_port, $length, 0, $pkt);
$delay = 0;
cpu_rxfifo_rd_pkt($in_port, $length, 0, $pkt);
cpu_rxfifo_rd_pkt($in_port, $length, 0, $pkt);
#nf_PCI_read32($delay, $batch, CPU_REG_Q_1_RD_NUM_PKTS_IN_Q_REG(), 0);
#nf_PCI_read32($delay, $batch, CPU_REG_Q_1_RX_NUM_PKTS_RCVD_REG(), 3);
