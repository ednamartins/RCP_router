...
sub get_q_num_pkts_reg {
	my $i = shift;
	if ($i == 0) { return OQ_QUEUE_0_QUEUE_NUM_PKTS_IN_Q(); }
	if ($i == 1) { return OQ_QUEUE_2_QUEUE_NUM_PKTS_IN_Q(); }
	if ($i == 2) { return OQ_QUEUE_4_QUEUE_NUM_PKTS_IN_Q(); }
	if ($i == 3) { return OQ_QUEUE_6_QUEUE_NUM_PKTS_IN_Q(); }	
}
